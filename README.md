...
Left-to-right flow. A **human operator** connects through one chat channel to the **Cadre Wizard**, which runs **once at install** to confirm roles and budgets. The flow then drops into the pipeline: **Requirements Analyst** (intake, clarify, scope) → **Project Designer** (architecture, approach, specifications) → **Project Manager** (plans, dispatches, tracks; **the workers are in its scope**) → **QA / Verifier** (independent check, *evidence before claims*; if the requirements aren't met, an option to return the work to the **Project Manager**) → **finished product**. A dashed red **rework** path returns from the **finished product** all the way to the Requirements Analyst, and two shorter gates catch problems earlier — **QA → PM** when requirements aren't met, and **PM → Requirements Analyst** when planning surfaces a new requirement: a scope change or new feature, or a new product, runs the pipeline again rather than being patched mid-stream. Along the bottom, a dashed panel lists **support roles on call as needed** — Security/IT, Consultant, Social Media Manager, Finance Manager, Agent Resources — beside the line, never on it.
