...
3. **Project Manager** — owns the plan and dispatches. **The workers sit in the PM's scope**: the PM tasks and tracks them, rather than the operator driving each one by hand. Planning may also surface a requirement intake missed; that returns to the Requirements Analyst, not into the plan by stealth.
...
**A worker is a seat, not a throwaway run.** Every worker the PM dispatches to is a **persistent agent in its own right** — its own workspace, persona files, memory, session store and mailboxes — exactly as the PM or QA has. It keeps its identity and its accumulated context across projects, so a worker gets better at its job the way the rest of the team does. The disposable mechanism is the **spawned subagent**: bounded scratch work any agent may spin up and discard, which is a *different thing* from the worker seat. Conflating the two is the same confusion §3 separates at the species level — a pipeline's "worker" is an ephemeral node; a team's worker is a colleague. Cadre recommends **two** of them by default: one worker is a single point of failure, and two lets the PM run parallel work and keep the line moving while one is blocked, reviewing, or mid-task.
